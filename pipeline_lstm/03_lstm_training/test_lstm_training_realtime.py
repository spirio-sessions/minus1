...
model = LSTMModel_Realtime(input_size, hidden_size, num_layers, output_size).to(device)
...
for epoch in range(num_epochs):
    ...
    for melodies, harmonies in train_loader:
        ...
        batch_size_actual = melodies.size(0)

        # Initialize hidden and cell states
        h_0 = torch.zeros(num_layers, batch_size_actual, hidden_size).to(device)
        c_0 = torch.zeros(num_layers, batch_size_actual, hidden_size).to(device)

        outputs, _ = model(melodies, (h_0, c_0))
